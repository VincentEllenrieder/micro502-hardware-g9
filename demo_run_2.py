...
import os
import pandas as pd
from demo_get_waypoints import csv_to_waypoints_motion_planner
...
WAYPOINTS2LAPS, TAKE_OFF_COORD = csv_to_waypoints_motion_planner(CSV_FILE_PATH)     # Load waypoints
RACING_VELOCITY = 0.7                                                               # Velocity goal during the racing, in m/s
PLANNER_STEP = 10                                                                   # Number of steps to divide every path segment
...
class MotionPlanner3D():
    ...
    def plot(self, obs, path_waypoints, trajectory_setpoints):
        ...
        ax.legend()
        plt.savefig("demo_motion_planner_trajectoy.png", dpi=600)
        plt.close()
    ...
    def plot_with_yaw(self, obs, path_waypoints, trajectory_setpoints):
        ...
        plt.savefig("demo_motion_planner_trajectoy_yaw.png", dpi=600)
        #plt.show()
        plt.close()
